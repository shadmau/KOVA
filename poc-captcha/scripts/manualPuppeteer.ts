...
const easyOCRPythonPath = path.resolve('../llms/easyOCR/easyocr-env/bin/python3');
const easyOCRScript = path.resolve('../llms/easyOCR/detect_text.py');

const yolov8PythonPath = path.resolve('../llms/yolov8/yolov8-env/bin/python3');
const yolov8Script = path.resolve('../llms/yolov8/detect_captcha.py');
...
function runPythonScript(pythonPath: string, scriptPath: string, imagePath: string) {
  return new Promise((resolve, reject) => {
    ...
    process.stdout.on('data', (data) => {
      ...
      // console.log(`PYTHON OUTPUT: ${data.toString()}`);  // Debugging
    });
...
/**
 * This script demonstrates how to solve a reCAPTCHA v2 challenge with YOLOv8, EasyOCR and puppeteer
 * Issues to identify: crosswalks, bridges, palmtrees, blurry images, reoccuring captchas, captchas to skip; See (captcha-examples/hard)
 * Limited support for 4x4 grids
 */
    ...
    if (recaptchaFrame) {
      ...
      if (detectedObjects.length === 0) {
        ...
        if (matchingObjects.length > 0) {
          //hardcoded boxes for 3x3 and 4x4 grids - should be dynamic
          const boxes3x3 = [
            // First row 
